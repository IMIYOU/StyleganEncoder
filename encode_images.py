...
def main():
    ...
    for images_batch in tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size):
        ...
        for img_array, dlatent, img_name in zip(generated_images, generated_dlatents, names):
            ...
            img.save(os.path.join(args.generated_images_dir, f'{img_name}_1.png'), 'PNG')
            np.save(os.path.join(args.dlatent_dir, f'{img_name}_1.npy'), dlatent)
            with open(os.path.join(args.dlatent_dir, f'{img_name}_1.txt'), 'w') as f:
                text_save(f, dlatent)
